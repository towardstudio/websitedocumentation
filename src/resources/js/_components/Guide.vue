<template>
	<nav id="sidebar-menu">
		<div class="nav-item" v-for="(section, index) in sections" :key="section.title">
			<div class="nav-item__title" data-parent @click="switchSection($event.target)" :data-sub="[section.children.length ? true : false]" :data-section="kebabCase(section.title)">
				<span class="icon">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 166.4 122.4">
						<path d="M104.2,0L83.6,8.7c10.9,23.5,24.9,35.3,43.6,42.9H0V71h126.7
				...
				<span class="chevron" v-if="section.sub">
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 90 90">
						<polygon points="49.7,69.7 39.1,69.6 2,30.5 12.7,20.4 44.5,54 77.5,20.3 88,30.6 "/>
					</svg>
				</span>
			</div>
			<div :data-submenu="kebabCase(section.title)"
				v-if="section.children.length">
				<div v-for="item in section.children"
					:key="item">
					<div class="nav-item__title nav-item__title--sub"
					@click="switchSection($event.target)" :data-section="kebabCase(item)" data-sub="false">
						<span class="icon">
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 166.4 122.4">
								<path d="M104.2,0L83.6,8.7c10.9,23.5,24.9,35.3,43.6,42.9H0V71h126.7
									c-18.2,8.7-32.4,19.4-43.1,42.6l20.6,8.7c22.5-46.2,45.7-50.9,62.3-53.7V53.7C149.9,50.9,126.7,46.2,104.2,0"/>
 	...
 	export default {
		...
		methods: {
			...
			switchSection(item) {
				...
				let parent = item.hasAttribute('data-parent') ? item.dataset.section : item.closest('[data-submenu]').dataset.submenu;
			...
			hashtagActive(hash, section) {
				...
				if (parentSub) {
					...
					const parentItem = document.querySelector('[data-section="'+ parentSub.dataset.submenu +'"]');
			...
			toggleActiveSection(section, item) {
				...
				if (item) {
					navSections.forEach(function (navSection) {
						...
						parentItem = parentItem && parentItem.hasAttribute('data-child-sub') ? parentItem.previousElementSibling : null;
			...
			findActiveContent(activeSection) {
				...
				const activeSections = sidebarMenu.querySelectorAll("." + activeSectionClass);
